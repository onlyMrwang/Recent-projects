...
<template>
  <div class="app-container" style="">
    ...
    <!-- 配置弹框 -->
    <el-dialog title="重启" :visible.sync="open" width="700px" append-to-body>
      <el-form
        :rules="rules"
        ref="form"
        :model="dialogform"
      ...
      >
        <el-row>
          <el-col :span="14">
            <el-form-item label="终端重启控制" prop="restartControl">
              <el-radio-group v-model="form.restartControl">
                <el-radio label="重启"></el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        </el-row>
...
// import {toIssue,list} from "@/api/remotecontrol/chargingpile/restart.js"
import {
  toIssue,
  getChargGroupCode,
  getEquipment,
  list
} from '@/api/remotecontrol/chargingpile/restart.js'
export default {
  name: 'Workparams',
  data() {
    return {
      ...
      rules: {
        restartControl: [{ required: true, message: '', trigger: 'change' }]
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    // 点击下发
    down() {
      toIssue({ deviceList: this.issueList }).then((res) => {
        if (res.code == 200) {
