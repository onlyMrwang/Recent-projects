      ...
      >
        ...
        >
          <el-row style="display:none">
            <el-col :span="14">
              <el-form-item label="通信模式" prop="communication">
                <el-radio-group v-model="form.communication">
                  <el-radio label="应答模式"></el-radio>
                  <el-radio label="上报模式"></el-radio>
                </el-radio-group>
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row>
            <el-col>
              <el-form-item label="定时上报时间(时间)" prop="reportingInterval">
                <el-input
                  v-model="dialogform.reportingInterval"
                  style="width:200px"
                  clearable
                  size="small"
                />
...
import {
  ...
  getEquipment,
  list
} from '@/api/remotecontrol/chargingpile/communicationparameter.js'
export default {
  name: 'Communication',
  data() {
    let number = (rule, value, cb) => {
      let reg = /^([1-9]|[1-9][0-9]|[1-2][0-9][0-9]|300)$/
      if (!reg.test(value)) {
        cb(new Error('上报时间要在1到300秒'))
      } else {
        cb()
      }
    }
    ...
    down() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          obj.communicationMode = this.dialogform.communicationMode
          obj.reportingInterval = this.dialogform.reportingInterval
